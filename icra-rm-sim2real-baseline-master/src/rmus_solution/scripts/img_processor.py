...
class Processor:
    def __init__(self, initial_mode=0, verbose=True) -> None:
        ...
        while not rospy.is_shutdown():
            try:
                ...
                rospy.logwarn("Waiting for message /camera/color/image_pose_listraw.")
                continue
        ...
        self.pub_id =rospy.Publisher("/see_blockid",UInt8MultiArray,queue_size=1)
        self.placed_pub_id =rospy.Publisher("/placed_blockid",UInt8MultiArray,queue_size=1)
    ...
    def imageCallback(self, image):
        ...
        elif locked_current_mode == 10:
            id_list = self.get_placed_info(self.image)
            if id_list is None:
                pass
            else:
                self.placed_pub_id.publish(UInt8MultiArray(data=id_list))
    ...
    def get_gameinfo(self, image):
        ...
        id_list, quads_list, area_list, tvec_list, rvec_list, _, _ = marker_detection(
            image, camera_matrix=self.camera_matrix, area_filter_size=150, verbose=self.verbose, exchange_station=1
        )
        number_dict = {}
        ...
        if len(number_dict) == 3:
            ...
            return None
    def get_placed_info(self, image):
        gameinfo = [0, 0, 0]
        id_list, quads_list, area_list, tvec_list, rvec_list, _, _ = marker_detection(
            image, camera_matrix=self.camera_matrix, area_filter_size=300, verbose=self.verbose, exchange_station=2
        )
        id_unint=[]
       
        for id, quads in zip(id_list, quads_list):
            if id <= 5 and id >= 1:
                if id not in id_unint:
                    id_unint.append(id)  
    ...
    def update_uint32_data(self, blockid):
        ...
        if unique_id != None:
            self.pub_id.publish(UInt8MultiArray(data=unique_id))
