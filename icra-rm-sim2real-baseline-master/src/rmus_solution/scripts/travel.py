...
class check_place(py_trees.behaviour.Behaviour):
    def __init__(self,name="check_palce",goal_places=[]):
        ...
        for goal_place in goal_places:
            places_inMB.append([False,self.creat_goal(goal_place[0],goal_place[1],goal_place[2]),goal_place[3]])
    ...
    def update(self):
        ...
        if now_place_inMB[self.index][0]==False:
            ...
            self.blackboard.set("next_rotate_time",now_place_inMB[self.index][2])
...
class rotate(py_trees.behaviour.Behaviour):
    ...
    def initialise(self):
        ...
        self.rotate_time=self.blackboard.get("next_rotate_time")
        self.begin_time=rospy.get_time()
        self.rotate_vel=0.31
        if self.rotate_time<0:
            self.rotate_time=-self.rotate_time
            self.rotate_vel=-0.31
    def update(self):
        twist=Twist()

        if (rospy.get_time()-self.begin_time)>self.rotate_time:
...
class goto_visi_place(action_behavior):
    def __init__(self):
        super().__init__(name="goto_visi_place",action_namespace="/move_base",override_feedback_message_on_running="moving")
    def initialise(self):
        blackboard=py_trees.blackboard.Blackboard()
        visi_place=blackboard.get("goto_visi_place")
        print("vis")
        goal_pose=move_base_msgs.MoveBaseGoal()
        goal_pose.target_pose.header.frame_id='map'
        goal_pose.target_pose.pose.orientation=visi_place.pose.orientation
        goal_pose.target_pose.pose.position.x=visi_place.pose.position.x
        goal_pose.target_pose.pose.position.y=visi_place.pose.position.y
        goal_pose.target_pose.pose.position.z=0
        self.action_goal=goal_pose
        print(self.action_goal)
        self.sent_goal=False
